  ...
  dashboardHeader(title = "Foreign Insight"),
    dashboardSidebar(
      sidebarMenu(id = "tabs",
        ...
        sliderInput("selectYear", "", min = 2012, max = 2020, value = 2020, sep = "")
    )
  ),
  dashboardBody(
    fluidRow(
        ...
        tabItem(tabName = "tab1",
                box(width = 3, title = "Census Sections", status = "primary", solidHeader = TRUE,
                    ...
                    selectizeInput("selectMunicipio", label = "Select Municipality",
                                   choices = c("Cartagena (Murcia)"), multiple = T, selected = "Madrid (Madrid)"),
                    selectInput("selectNacionalidad", label = "Select Nationality",
                                choices = levels(SXnacionalAmbos$nacionalidad), selected = "Total Población"),
